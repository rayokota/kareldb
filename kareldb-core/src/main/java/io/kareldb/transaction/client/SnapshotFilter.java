...
public interface SnapshotFilter {
    ...
    List<VersionedValue> get(KarelDbTransaction transaction, Comparable[] key);

    KeyValueIterator<Comparable[], List<VersionedValue>> range(
        KarelDbTransaction transaction, Comparable[] from, boolean fromInclusive, Comparable[] to, boolean toInclusive);

    KeyValueIterator<Comparable[], List<VersionedValue>> all(KarelDbTransaction transaction);
}
